...
def main():
    ...
    while choice =='0':
        ...
        energySystems=loadExistingSystems()
        displayAll(energySystems)
        ...
        if choice == "3":
            ...
            energySystem= addEnergySystem()
            energySystems.append(energySystem)
            print("All your energy systems summary:\n")
            displayAll(energySystems)
            saveToTheFile(energySystems)
            choice=input("Press 0 to go back to main menu.\n")
        elif choice == "1":
...
def addEnergySystem():
    ...
    energySystem["governate"]=governate
    energySystem["use"]=energy
    energySystem["area"]=area
    energySystem["well"]=well
    energySystem["connected"]=connected
    
    if((governate=="Aleppo" or governate=="Damascus") and (energy=="home" or energy=="shop") and well=="n"):
        print("You can only install solar Panels.\n")
        energySystem["installationType"]="solar panels"
        calc=area*10/120
        print(calc)
        print("KW\n")
        if(calc<5):
        ...
        totalcost=400+250+150+price
        print("Total cost=",totalcost)
        energySystem["price"]=price
        energySystem["cost"]=totalcost
    elif((governate=="Aleppo" or governate=="Damascus") and (energy=="home" or energy=="shop") and well=="y"):
        print("You can install a hybrid system.\n")
        energySystem["installationType"]="hybrid system"
        calc=area*10/120
        print(calc)
        print("KW\n")
        if(calc<5):
        ...
        print("You can install both solar system or wind turbine(recommended)\n")
        energySystem["installationType"]="solar system or wind turbine(recommended)"
        calc=area*10/120
        print(calc)
        print("KW\n")
        if(calc<5):
        ...
        print("We will come and check your location and which System is best for your comfort.\n")
    return energySystem
def saveToTheFile(energySystems):
    f=open("energySystems.json","w")
    f.write(json.dumps(energySystems,indent=2))
    f.close()
def loadExistingSystems():
    try:
        with open("energySystems.json") as data:
            energySystems=json.load(data)
            return energySystems
    except IOError:
        print ("No history found")
        return []
def displayAll(energySystems):
    for currSys in energySystems:
        for currKey in currSys:
            print(currKey,":",currSys[currKey])
        print("")
